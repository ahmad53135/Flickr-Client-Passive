...
public class FlickrConnection {
    ...
    private static final String key = "fa5315e9c69d34c93da13d0ed799ca84";
    private static final String secret = "af30db320bc79016";
    private static final String user = "ahmadsepahi";
    private static final String myNsid = "156791166@N05";
    ...
    private void authorize() throws IOException, SAXException, FlickrException {
        //Token token = (Token)"&secret=f7de261d4dd6aabc";

        //Token reqToken = this.flickr.getAuthInterface().getRequestToken();


        Token reqToken =new Token("&secret=f7de261d4dd6aabc","af30db320bc79016","200");
        //this.flickr.getAuthInterface().checkToken(reqToken);

        //OAuthService service = (new ServiceBuilder()).provider(FlickrApi.class).apiKey(key).apiSecret(this.sharedSecret).callback(null).build();

        //new ServiceBuilder().provider(Api.class).apiKey(key).apiSecret(secret).callback(null).build();
...
*/
    ...
    public static void main(String[] args) throws IOException, SAXException, FlickrException, ParserConfigurationException {
        ...
        File file = new File("C:\\Users\\Ahmad\\Desktop\\download.png");
        fc.connect();
        fc.upload(file);
        fc.getPhotoList();
    }
...
//            System.out.println("We've used "+Auth.getBandwidthUsed()+f
